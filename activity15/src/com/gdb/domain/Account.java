...
public abstract class Account implements IAccount {
    ...
    public double getDailyTransferLimit() {
        ...
        // Get the daily transfer limit from the AccountRulesEngine.
    ...
    public double getRemainingDailyTransferLimit() {
        ...
        // Make sure the daily total belongs to today.
        resetDailyTransferIfNeeded();

        // Calculate the remaining amount.
        // Math.max() prevents the value from going below zero.
        return Math.max(
                0.0,
    ...
    public boolean canTransfer(double amount) {
        ...
        // Check whether the new transfer stays within the daily limit.
    ...
    public void updateDailyTransferTotal(double amount) {
        ...
        // Make sure we are updating today's total.
        resetDailyTransferIfNeeded();

        // Add the successfully transferred amount.
    ...
    public void resetDailyTransferIfNeeded() {
        ...
        // We do not compare hours/minutes/seconds because transfers
        // made on the same calendar day should share the same total.
        if (!lastTransferDate.toLocalDate().equals(now.toLocalDate())) {

            // A new day has started, so reset today's transfer total.
